...
// DMBDocumentationBuilder.csproj DocumentationRecordPageRenderer.cs create at 2026/04/12 12:04:31
...
{
    /// <summary>
    /// Represents the DocumentationRecordPageRenderer type used by DocumentationBuilder generation.
    /// </summary>
    public static class DocumentationRecordPageRenderer
    {
...
";
        ...
        private static string RenderFieldBody(DocumentationRecordFieldItem field)
        {
        ...
        public static string RenderHtmlPage(
            DocumentationRecordPageModel item,
            string sharedDocumentationRootDirectory
        )
        {
            StringBuilder sb = new();
            ...
            if (item.IsObsolete)
            {
                ...
                sb.Append("                    <div class=\"small\">")
                    .Append(string.IsNullOrWhiteSpace(item.ObsoleteMessage) ? "This record is obsolete." : Html(item.ObsoleteMessage))
                    .AppendLine("</div>");
                sb.AppendLine("                </div>");
            }
            ...
            sb.Append("                            <dd class=\"col-sm-9\"><code>")
                .Append(item.IsRecordStruct ? "Record struct" : "Record")
                .AppendLine("</code></dd>");
            ...
            if (item.Fields.Count > 0)
            {
                ...
                foreach (DocumentationRecordFieldItem field in item.Fields)
                {
                    sb.Append("                            <li class=\"list-group-item px-0 py-3 my-2\" ")
            ...
            if (item.Properties.Count > 0)
            {
                ...
                foreach (DocumentationRecordPropertyItem property in item.Properties)
                {
                    sb.Append("                            <li class=\"list-group-item px-0 py-3 my-2\" ")
            ...
            if (item.Methods.Count > 0)
            {
                ...
                foreach (DocumentationRecordMethodItem method in item.Methods)
                {
                    sb.Append("                            <li class=\"list-group-item px-0 py-3 my-2\" ")
        ...
        private static string RenderMethodBody(DocumentationRecordMethodItem method)
        {
            StringBuilder sb = new();
        ...
        public static void RenderPages(
            IEnumerable<DocumentationRecordPageModel> items,
        ...
        )
        {
            ...
            foreach (DocumentationRecordPageModel item in items)
            {
        ...
        private static string RenderPropertyBody(DocumentationRecordPropertyItem property)
        {
            StringBuilder sb = new();

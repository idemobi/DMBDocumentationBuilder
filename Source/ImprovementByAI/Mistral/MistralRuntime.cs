...
{
    /// <summary>
    ///     Runs Mistral documentation improvement against a generated documentation database.
    /// </summary>
    public static class MistralRuntime
    {
        ...
        private sealed class DocumentationDatabaseImprover
        {
            ...
            public async Task<Dictionary<long, string>> LoadMistralHashesAsync(CancellationToken ct)
            {
                using var connection = new SqliteConnection($"Data Source={_aiDatabasePath}");
                await connection.OpenAsync(ct);

                EnsureMistralTable(connection);
            ...
            /// <summary>
            ///     Inserts or updates Mistral AI output for a documentation object.
            /// </summary>
...
";
        ...
        private static string GetAIDatabasePath(string mainDatabasePath, MistralModel model)
        {
            ...
            string fileName = $"Mistral_{model}.db";

            return Path.Combine(mainDirectory, "AI", "Mistral", fileName);
        }

        private static void EnsureMistralTable(SqliteConnection connection)
        {
            const string sql = @"
...
";
        ...
        /// <summary>
        ///     Runs Mistral documentation improvement synchronously.
        /// </summary>
        public static void Run(MistralOptions options)
        {
            Console.WriteLine("Starting Mistral runtime...");
            RunAsync(options).GetAwaiter().GetResult();
            Console.WriteLine("Finished Mistral runtime.");
        }

        /// <summary>
        ///     Runs Mistral documentation improvement asynchronously.
        /// </summary>
        /// <returns>A task that completes when documentation improvement finishes.</returns>
        public static async Task RunAsync(MistralOptions options)
        {
            ...
            using (var bootstrapConnection = new SqliteConnection($"Data Source={aiDatabasePath}"))
            {
                ...
                EnsureMistralTable(bootstrapConnection);
            }
            ...
            MistralModel currentModel = options.Model;
            string currentModelName = currentModel.ToModelString();

            Console.WriteLine($"[MISTRAL] Main DB: {mainDatabasePath}");
            Console.WriteLine($"[MISTRAL] AI DB:   {aiDatabasePath}");
            Console.WriteLine($"[MISTRAL] Model:   {currentModelName}");
            ...
            foreach (var row in rows)
            {
                ...
                try
                {
                    ...
                    Console.WriteLine($"[MISTRAL n° {i}] Processing {row.NamespaceName}.{row.ObjectName} ({row.ObjectType})");
                ...
                catch (Exception exception)
                {
                    ...
                    Console.WriteLine($"[MISTRAL][ERROR] Failed for {row.NamespaceName}.{row.ObjectName} ({row.ObjectType})");

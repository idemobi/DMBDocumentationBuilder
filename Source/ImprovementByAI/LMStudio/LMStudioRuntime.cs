...
{
    /// <summary>
    ///     Runs LM Studio documentation improvement against a generated documentation database.
    /// </summary>
    public static class LMStudioRuntime
    {
        ...
        /// <summary>
        ///     Runs LMStudio documentation improvement synchronously.
        /// </summary>
        public static void Run(LMStudioOptions options)
        {
            Console.WriteLine("Starting LM Studio runtime...");
            RunAsync(options).GetAwaiter().GetResult();
            Console.WriteLine("Finished LM Studio runtime.");
        }

        /// <summary>
        ///     Runs LMStudio documentation improvement asynchronously.
        /// </summary>
        /// <returns>A task that completes when documentation improvement finishes.</returns>
        public static async Task RunAsync(LMStudioOptions options)
        {
            ...
            using (var bootstrapConnection = new SqliteConnection($"Data Source={aiDatabasePath}"))
            {
                ...
                EnsureLMStudioTable(bootstrapConnection);
            }

            RegisterRenderSource(
                mainDatabasePath,
                provider: "LMStudio",
                model: options.Model.ToStorageKey(),
                aiDatabasePath: aiDatabasePath);

            using var cancellationTokenSource = new CancellationTokenSource();
            CancellationToken cancellationToken = cancellationTokenSource.Token;

            LMStudioModel currentModel = options.Model;
            ...
            Console.WriteLine($"[LMSTUDIO] {rows.Count} object(s) loaded.");
            ...
            foreach (DocumentationObjectRow row in rows)
            {
                ...
                try
                {
                    ...
                    Console.WriteLine($"[LMSTUDIO n° {i}] Processing {row.NamespaceName}.{row.ObjectName} ({row.ObjectType})");
                ...
                catch (Exception exception)
                {
                    ...
                    Console.WriteLine($"[LMSTUDIO][WARNING] Failed with model '{currentModelName}' for {row.NamespaceName}.{row.ObjectName} ({row.ObjectType})");
                    ...
                    RegisterRenderSource(
                        mainDatabasePath,
                        provider: "LMStudio",
                        model: currentModel.ToStorageKey(),
                        aiDatabasePath: GetAIDatabasePath(mainDatabasePath, currentModel));
                }
            }
        }
        ...
        private static string GetAIDatabasePath(string mainDatabasePath, LMStudioModel model)
        {
            ...
            string fileName = $"LMStudio_{model.ToStorageKey()}.db";

            return Path.Combine(mainDirectory, "AI", "LMStudio", fileName);
        }

        private static void EnsureLMStudioTable(SqliteConnection connection)
        {
            const string sql = @"
...
";
        ...
        private sealed class DocumentationDatabaseImprover
        {
            ...
            public async Task<Dictionary<long, string>> LoadLMStudioHashesAsync(CancellationToken cancellationToken)
            {
                using var connection = new SqliteConnection($"Data Source={_aiDatabasePath}");
                await connection.OpenAsync(cancellationToken);

                EnsureLMStudioTable(connection);
            ...
            /// <summary>
            ///     Inserts or updates LMStudio AI output for a documentation object.
            /// </summary>

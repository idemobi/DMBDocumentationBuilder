...
// DMBDocumentationBuilder.csproj DocumentationGroupTechnicalKeywordExtractor.cs create at 2026/04/13 18:04:39
...
{
    /// <summary>
    /// Represents the DocumentationGroupTechnicalKeywordExtractor type used by DocumentationBuilder generation.
    /// </summary>
    public static class DocumentationGroupTechnicalKeywordExtractor
    {
        #region Fields
        ...
        public static string ExtractKeywordsAsString(DocumentationGroupPageModel model)
        {
        ...
        public static IReadOnlyList<string> ExtractKeywords(DocumentationGroupPageModel model)
        {
            ...
            foreach (string namespaceName in model.NamespaceNames)
            {
                Add(keywords, namespaceName);
            }

            foreach (DocumentationNamespaceObjectLinkItem namespaceLink in model.NamespaceLinks)
            {
                Add(keywords, namespaceLink.PackageId);
                Add(keywords, namespaceLink.Version);
                Add(keywords, namespaceLink.Name);
            }
